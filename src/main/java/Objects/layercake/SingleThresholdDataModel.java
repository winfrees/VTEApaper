...
import Exploration.Datasets;

import Objects.MicroObjectModel;
import Objects.layercake.RegionFactory;
import ij.IJ;
...
public class SingleThresholdDataModel implements Datasets, MicroObjectModel{
    ...
    public SingleThresholdDataModel(ImageStack[] is, List details) {
        ...
        float[] minConstants = new float[4];
	// 0: minObjectSize, 1: maxObjectSize, 2: minOverlap, 3: minThreshold
        ...
        builderVolumes.makeDerivedRegions(derivedRegionType, is.length, is, getResultsPointers(details));

        //IJ.log("SimpleThresholdModel, derived regions: " + derivedRegionType);

        //derivedRegionType[][], [Channel][0, type, 1, subtype];
        //int[][] derivedRegionType = new int[3][2];
        Volumes = builderVolumes.getVolumesAsArrayList();
